...
var copyColorEnabled = false; // new new woohoo
var copyCellColorEnabled = false; // also new new woohoo
var copyDecoEnabled = false; // final new new woohoo
...
function event_keydown_copy_char(e) {
	...
	if (copyDecoEnabled) {
		const deco = getCharDecoration(tileX, tileY, charX, charY) || {};
		const BOLD = "⃸";
		const ITALIC = "⃴";
		const UNDER = "⃲";
		const STRIKE = "⃲";

		if (deco.bold) styledChar += BOLD;
		if (deco.italic) styledChar += ITALIC;
		if (deco.underline) styledChar += UNDER;
		if (deco.strike) styledChar += STRIKE;
	}

	let output = styledChar;

	if (copyColorEnabled) {
		const textColor = getCharColor(tileX, tileY, charX, charY);
		const textHex = '\x1bF' + textColor.toString(16).padStart(6, '0').toLowerCase();

		if (copyCellColorEnabled) {
			const bgColor = getCharBgColor(tileX, tileY, charX, charY);
			const bgHex = '\x1bb' + bgColor.toString(16).padStart(6, '0').toLowerCase();

			if (bgHex !== '\x1bb0000-1') {
				output = bgHex + '\x1bx' + bgHex + textHex + styledChar;
			} else {
				output = textHex + styledChar;
			}
		} else {
			output = textHex + styledChar;
		}
...
function buildMenu() {
	...
	menuOptions.copyColor = menu.addCheckboxOption("Copy color", function () {
		copyColorEnabled = true;
		menu.showEntry(menuOptions.copyCellColor);
	}, function () {
		copyColorEnabled = false;
		menu.hideEntry(menuOptions.copyCellColor);
	});
	menuOptions.copyCellColor = menu.addCheckboxOption("Copy cell color", function () {
		copyCellColorEnabled = true;
	}, function () {
		copyCellColorEnabled = false;
	});
	menu.hideEntry(menuOptions.copyCellColor);
	menuOptions.copyDeco = menu.addCheckboxOption("Copy decorations", function () {
		copyDecoEnabled = true;
	}, function () {
		copyDecoEnabled = false;
	})
	/**
	 * End of new
	 */
...
function updateWorldName() {
	...
	if (!name || name.toLowerCase() == "main" || name.toLowerCase() == "owot") {
		document.title = "My World of Text";
	} else {
